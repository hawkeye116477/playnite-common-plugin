...
"""
            if ftl_string != "":
                ...
                ftl_filename = "common"
                if "-" in filename and not "third-party" in filename:
                    ftl_filename = ftl_dir.split('-')[1]
                    ftl_dir = ftl_dir.split('-')[0]
                ftl_dir = ftl_dir.replace("_", "-")
                if "third-party" in filename:
                    ftl_filename = "third-party"
                    ftl_dir = os.path.basename(root)
                ...
                try:
                    ...
                    def get_new_key(old_key):
                        ...
                        if old_key_replaced in key_to_prefix_map:
                            ...
                            if prefix == "Common":
                                new_key = new_key.replace(f"LOC.{plugin_prefix}", "LOC.Common")
                            if prefix == "ThirdParty":
                                new_key = new_key.replace(f"LOC.{plugin_prefix}3P_", "LOC.ThirdParty")
                            return new_key
                        return old_key

                    def convert_xaml_string(match):
                        full_key = match.group(1)
                        if "3P" in full_key and not convert_3p:
            ...
            if filename.endswith(".cs"):
                ...
                def process_match_and_replace(match):
                    if match.group('string_key'):
                        ...
                        if f'LOC.{plugin_prefix}3P' in old_key and not convert_3p:
...
if __name__ == '__main__':
    plugin_prefix = "Legendary"
    script_path = os.path.dirname(os.path.realpath(__file__))
    main_path = pn(script_path+"/..")
    src_path = pj(main_path, "src")
    config_path = pj(script_path, "config")
    loc_path = pj(main_path, "third_party", "Localization")
    convertToFtl(plugin_prefix, src_path, config_path, loc_path, True, True)
